...
// Nov 13 2026, 6pm local — the countdown target (start of the Nov 13-15 weekend)
const EVENT_DATE = new Date(2026, 10, 13, 18, 0, 0);
...
function downloadInviteIcs() {
  ...
  const end = new Date(start.getTime() + 3 * 24 * 60 * 60 * 1000); // spans the Nov 13-15 weekend
  const ics = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Benji Balls' Birthday//EN",
    "BEGIN:VEVENT",
    `UID:benji-balls-bday-2026@event-invite`,
    `DTSTAMP:${icsStamp(new Date())}`,
    `DTSTART:${icsStamp(start)}`,
    `DTEND:${icsStamp(end)}`,
    "SUMMARY:Benji Balls' Birthday",
    `LOCATION:${CITY}`,
    "DESCRIPTION:Benji Balls is turning 27 — see the app for the full itinerary.",
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n");

  downloadBlob(new Blob([ics], { type: "text/calendar;charset=utf-8" }), "benji-balls-birthday.ics");
}
...
export default function BirthdayInvite() {
  ...
  // most events attending first — ties keep their original (submission) order since sort is stable
  const byAttendingCount = (a, b) => (b.attending?.length || 0) - (a.attending?.length || 0);
  const going = guests.filter((g) => g.status === "going").sort(byAttendingCount);
  const maybe = guests.filter((g) => g.status === "maybe").sort(byAttendingCount);
    ...
    >
      <style>{`
        :root {
          ...
          --accent-a: #3E5C94;
          --accent-b: #E5484D;
          --glow-a: rgba(62,92,148,0.34);
          --glow-b: rgba(229,72,77,0.26);
        }
        ...
        .boot-bottle-body {
          ...
          background: linear-gradient(160deg, rgba(116,167,111,0.95), rgba(53,104,47,0.98));
        ...
        .boot-stream {
          ...
          background: linear-gradient(to bottom, #FFFFFF, #C4C8CC);
        ...
        .boot-glass-fill {
          ...
          background: linear-gradient(180deg, #FFFFFF, #C4C8CC);
      ...
      {booting &&
        ...
        createPortal(
          <div className={`boot-overlay ${bootFading ? "boot-fade" : ""}`}>
            ...
            <div className="boot-label">Pouring up the soju…</div>
              ...
              {showMachine && (
                <>
                  ...
                  <div className={`machine-body ${stage === "printing" ? "machine-shake" : ""}`}>
                    ...
                    <div className="machine-label">NOV 13–15 • ATLANTA</div>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="page-content">
            ...
            <div className="glass-panel" style={{ padding: "30px 26px", width: "100%", marginTop: 24 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "var(--muted)", marginBottom: 20, flexWrap: "wrap" }}>
                <Calendar size={13} /> Nov 13–15, 2026
  ...
  const front = (
    <div className={`ticket-metal ${docked ? "ticket-metal-docked" : ""} ${expanded ? "ticket-metal-expanded" : ""}`}>
      ...
      <div className="ticket-eyebrow">
        <span>Benji Balls</span>
        <span className="ticket-admit">Admit One</span>
      </div>
      <div className="ticket-ages">27</div>
